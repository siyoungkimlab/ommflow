...
- Python 3.12, since `install.sh` pins numpy below 2, which has no builds for
  3.13 or later
...
bash install.sh --cuda 12         # same, with OpenMM built for CUDA 12
```
...
baseline optimizations (X86_V2) but your machine doesn't support"*, and
AmberTools' bundled tools pin `numpy<2` themselves.
...
Pass `--cuda` with the CUDA release to build for, the same choice OpenMM's own
instructions make with `cuda-version=12` and `openmm[cuda12]`:
...
NVIDIA driver is present. Without one it says why CUDA did not load; confirm
on a GPU node with `python -m openmm.testInstallation`.
...
Python 3.12. `pyproject.toml` leaves
